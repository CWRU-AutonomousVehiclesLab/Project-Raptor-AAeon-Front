...
class cannyEdge {
  ...
  public:
	cannyEdge() : it(nh) {
		imgSub = it.subscribe("/front_realSense/color/image_raw", 1, &cannyEdge::process, this);
		imgPub = it.advertise("/front_realSense/cv_processed/canny_raw",1);
		imgPubOverlay = it.advertise("/front_realSense/cv_processed/canny_overlay",1);
	}
...
int main(int argc, char** argv) {
	ros::init(argc, argv, "canny_edge_output");
	cannyEdge worker;
	ros::spin();
